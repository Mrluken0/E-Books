...
def main():
    ...
    Types de livre : {author.get("Type de livre", "")}
    Interdits : {author.get("Ce qu'il n'est pas", "")}
    """
    ...
    for _, row in df_scores.iterrows():
        ...
        if any(x in category_lower for x in excluded):
            continue

        titles = df_data[df_data["category"] == category]["title"].dropna().head(10).tolist()
        titles_text = " | ".join(titles)

        category_text = f"""
        Catégorie Amazon : {category}
        Titres représentatifs : {titles_text}
        ...
        final_score = round((semantic_score * 100) + float(row.get("kdp_score", 0)) * 0.3, 2)
    ...
    output_file = OUTPUT_DIR / f"semantic_niches_by_author_{safe_author}_{date_str}.xlsx"
